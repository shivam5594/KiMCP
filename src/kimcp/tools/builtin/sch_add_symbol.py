...
class SchAddSymbolTool(Tool[SchAddSymbolInput, SchAddSymbolOutput]):
    ...
    version = "0.2.0"
    description = (
        "Place a symbol instance on a .kicad_sch schematic. Requires the "
        "symbol's lib_id to be already embedded in lib_symbols. Generates "
        "UUIDs, synthesizes default properties (Reference/Value/Footprint/"
        "Datasheet), and emits one pin entry per pin in the lib symbol. "
        "Plan coordinates on KiCAD's 100-mil schematic grid (2.54 mm "
        "default; see safety.grid_snap_mm) — symbol pins are designed on "
        "this grid, so an on-grid origin keeps every pin landing on a "
        "wire-routable point and prevents endpoint_off_grid ERC warnings. "
        "Off-grid inputs are snapped (cites KICAD-318) and a meta.warnings "
        "entry is emitted. Also emits a meta.warnings entry when the new "
        "placement is within safety.symbol_spacing_warn_mm (default 5.08 mm) "
        "of an existing symbol on the same sheet (cites KICAD-317) — spread "
        "components generously, especially within functional blocks, so "
        "labels and bodies don't overlap. Supports dry_run; snapshots the "
