...
class SchComposeTool(Tool[SchComposeInput, SchComposeOutput]):
    ...
    description = (
        ...
        "single parse + snapshot + save cycle. Prefer this over chained "
        "individual sch_add_wire/sch_add_junction/sch_add_label/sch_add_symbol "
        "calls whenever you have ≥3 mutations queued: per-call git "
        "snapshot + sexpr re-parse costs ~225 ms per call independent of "
        "what the call does, so batching 30 junctions into one compose "
        "step saves ~6 s of fixed overhead alone. Supports five step ops "
        "via a discriminated union on 'op': add_wire, add_junction, "
        "add_no_connect, add_label, add_symbol. Steps run in order; later "
        "steps see earlier steps' mutations. Set continue_on_error=true to "
        "treat the batch as best-effort (the default aborts on first "
        "failure without writing anything). For add_symbol, the lib_id "
        "must already be embedded via sch_embed_lib_symbol — compose does "
        "not auto-embed. Power-port placement (sch_add_power) and "
        "hierarchical sheet creation (sch_add_sheet) are NOT in this v1 "
        "due to their file-side-effect logic; call those one-at-a-time. "
