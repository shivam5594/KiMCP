...
class SchComposeTool(Tool[SchComposeInput, SchComposeOutput]):
    ...
    description = (
        ...
        "CONNECTIVITY RULE: Connect nearby pins with add_wire, NOT "
        "add_label. Labels are net-name references for: (1) cross-sheet "
        "connectivity, (2) distinguishing ground domains (AGND/DGND), "
        "(3) naming important signals (CLK, SDA). A typical sheet has "
        "single-digit label count. If two pins are on the same sheet and "
        "within wire-routing distance, use add_wire with orthogonal H/V "
        "segments + add_junction at T-connections. Power symbols (GND, "
        "+12V) placed via sch_add_power are fine — they are the standard "
        "way to mark power rails — but do NOT use add_label as a "
        "substitute for wires. "
        ...
        "queued. For add_symbol, the lib_id must already be embedded via "
