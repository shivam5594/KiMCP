...
class SchEmbedLibSymbolTool(Tool[SchEmbedLibSymbolInput, SchEmbedLibSymbolOutput]):
    ...
    description = (
        ...
        "LIBRARY SOURCING: Before creating custom symbols with "
        "lib_add_symbol, first check if the part exists in: (1) KiCAD's "
        "built-in libraries (lib_search_symbol), (2) online sources like "
        "SnapEDA, Ultra Librarian, or the manufacturer's website — these "
        "provide KiCAD-compatible .kicad_sym files with correct pinouts "
        "and footprints. Ask the user before searching online. Present "
        "a summary: which parts were found in existing libraries, which "
        "are available online for download, and which truly need custom "
        "creation. Downloaded symbols build the user's local library for "
        "future projects. Only create custom symbols as a last resort. "
        "Reads from a .kicad_sym file, renames the top-level symbol to "
        "lib-qualified form, and inserts into the schematic. CALL ONCE "
        "PER UNIQUE (lib_id, sch_path) PAIR per session — idempotent. "
