...
def _sch(tmp_path: Path) -> Path:
    s = tmp_path / "design.kicad_sch"
    s.write_text("(kicad_sch (version 20240108))\n", encoding="utf-8")
    return s
...
async def test_e2e_ok(tmp_path: Path, memory_transport_factory) -> None:
    stub = _stub(tmp_path)
    sch = _sch(tmp_path)
    out_path = tmp_path / "docs" / "review.pdf"

    server = Server(config=_cfg(tmp_path, str(stub)))
    server.register_tool(SchExportPdfTool())
