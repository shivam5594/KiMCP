"""End-to-end: JSON-RPC tools/call → pcb_export_drill against a kicad-cli stub.

Exercises the full M8 path: Server constructs ``CliBackend`` from config,
injects it into ``PcbExportDrillTool``, dispatcher gates on
...
stub writes fake drill files into the requested ``--output`` dir, and
...
architectural mirror as the ``pcb_drc`` and ``pcb_export_gerbers`` e2e
tests, since ``pcb_export_drill`` declares ``preferred_backends=(CLI,)``.

The shell stub is a near-duplicate of the one in
``tests/unit/test_tool_pcb_export_drill.py`` and the sibling
``tests/e2e/test_pcb_export_gerbers.py``. Keeping a local copy instead
of hoisting to ``conftest.py`` matches the rationale already documented
in ``test_tool_ipc_get_version.py`` — the stub is cheap, the test reads
...
async def test_tools_call_pcb_export_drill_ok(
    ...
    server.register_tool(PcbExportDrillTool())
    ...
    # `kind` hints round-trip through JSON serialization.
...
@pytest.mark.asyncio
async def test_tools_call_pcb_export_drill_dry_run(
    tmp_path: Path, memory_transport_factory
) -> None:
    ...
    stub = _write_kicad_cli_stub(tmp_path)
    pcb = _touch_pcb(tmp_path)
    out_dir = tmp_path / "drill_dry_e2e"
    ...
    assert "drill" in result["cli_argv"]
...
@pytest.mark.asyncio
async def test_tools_call_pcb_export_drill_backend_unavailable(
    tmp_path: Path, memory_transport_factory
) -> None:
    """Dispatcher raises BACKEND_UNAVAILABLE when Backend.CLI isn't marked.

    Pins the architectural choice: pcb_export_drill is backend-gated —
