"""E2E JSON-RPC round-trip for M21 sch_add_no_connect."""
...
async def test_tools_call_sch_add_no_connect_ok(
    ...
    assert result["no_connect_uuid"] is not None
    ...
    assert _find_no_connect_by_uuid(doc.root, result["no_connect_uuid"]) is not None


@pytest.mark.asyncio
async def test_tools_call_sch_add_no_connect_dry_run(
    tmp_path: Path, memory_transport_factory
) -> None:
    sch = _write_sch(tmp_path)
    ...
    assert result["no_connect_uuid"] is None
    assert sch.read_bytes() == before


@pytest.mark.asyncio
async def test_tools_call_sch_add_no_connect_backend_unavailable(
    tmp_path: Path, memory_transport_factory
) -> None:
    sch = _write_sch(tmp_path)
    server = Server(config=_cfg(tmp_path), project_root=tmp_path)
