"""E2E JSON-RPC round-trip for M17 sch_add_label."""
...
from kimcp.tools.builtin.sch_add_label import SchAddLabelTool, _find_label_by_uuid
...
async def test_tools_call_sch_add_label_ok(
    ...
    assert _find_label_by_uuid(doc.root, result["label_uuid"]) is not None


@pytest.mark.asyncio
async def test_tools_call_sch_add_label_dry_run(
    tmp_path: Path, memory_transport_factory
) -> None:
    sch = _write_sch(tmp_path)
    ...
    assert result["label_uuid"] is None
    assert sch.read_bytes() == before


@pytest.mark.asyncio
async def test_tools_call_sch_add_label_backend_unavailable(
    tmp_path: Path, memory_transport_factory
) -> None:
    sch = _write_sch(tmp_path)
    server = Server(config=_cfg(tmp_path), project_root=tmp_path)
