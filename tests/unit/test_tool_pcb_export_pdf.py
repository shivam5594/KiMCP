"""Unit tests for pcb_export_pdf (M24)."""
...
from kimcp.tools.builtin.pcb_export_pdf import (
    PcbExportPdfInput,
    PcbExportPdfTool,
)
...
def _make_tool(stub: Path) -> PcbExportPdfTool:
    tool = PcbExportPdfTool()
...
async def test_ok_defaults(tmp_path: Path) -> None:
    ...
    out = await tool.run(PcbExportPdfInput(pcb_path=pcb))
    assert out.status == "ok"
    assert out.output_path == str(pcb.with_suffix(".pdf").resolve())
...
async def test_custom_output_path(tmp_path: Path) -> None:
    ...
    tool = _make_tool(stub)
    out = await tool.run(PcbExportPdfInput(pcb_path=pcb, output_path=custom))
    assert out.status == "ok"
...
async def test_argv_default_flags(tmp_path: Path) -> None:
    stub = _write_kicad_cli_stub(tmp_path)
    pcb = _touch_pcb(tmp_path)
    await _make_tool(stub).run(PcbExportPdfInput(pcb_path=pcb))

    argv = _read_argv(stub)
    assert argv[:3] == ["pcb", "export", "pdf"]
    layers = argv[argv.index("--layers") + 1]
    for exp in ("F.Cu", "B.Cu", "Edge.Cuts"):
        assert exp in layers
    # Default mode is multipage.
    assert "--mode-multipage" in argv
    assert "--mode-single" not in argv
    # include_border_title defaults True.
    assert "--include-border-title" in argv
    # drill_shape='real' -> "2".
    ...
    # No common-layers by default.
    assert "--common-layers" not in argv
...
async def test_argv_custom_layers(tmp_path: Path) -> None:
    ...
    await _make_tool(stub).run(
        PcbExportPdfInput(pcb_path=pcb, layers=["F.Cu", "In1.Cu", "In2.Cu", "B.Cu"])
    )
    argv = _read_argv(stub)
    assert argv[argv.index("--layers") + 1] == "F.Cu,In1.Cu,In2.Cu,B.Cu"


@pytest.mark.asyncio
async def test_argv_common_layers(tmp_path: Path) -> None:
    stub = _write_kicad_cli_stub(tmp_path)
    pcb = _touch_pcb(tmp_path)
    await _make_tool(stub).run(
        PcbExportPdfInput(pcb_path=pcb, common_layers=["Edge.Cuts", "F.Fab"])
    )
    argv = _read_argv(stub)
    assert argv[argv.index("--common-layers") + 1] == "Edge.Cuts,F.Fab"


@pytest.mark.asyncio
async def test_argv_mode_single(tmp_path: Path) -> None:
    stub = _write_kicad_cli_stub(tmp_path)
    pcb = _touch_pcb(tmp_path)
    await _make_tool(stub).run(PcbExportPdfInput(pcb_path=pcb, mode="single"))
    argv = _read_argv(stub)
    assert "--mode-single" in argv
    assert "--mode-multipage" not in argv
...
async def test_argv_drill_shape_none(tmp_path: Path) -> None:
    stub = _write_kicad_cli_stub(tmp_path)
    pcb = _touch_pcb(tmp_path)
    await _make_tool(stub).run(PcbExportPdfInput(pcb_path=pcb, drill_shape="none"))
    argv = _read_argv(stub)
...
async def test_argv_drill_shape_small(tmp_path: Path) -> None:
    stub = _write_kicad_cli_stub(tmp_path)
    pcb = _touch_pcb(tmp_path)
    await _make_tool(stub).run(PcbExportPdfInput(pcb_path=pcb, drill_shape="small"))
    argv = _read_argv(stub)
...
async def test_argv_visual_flags(tmp_path: Path) -> None:
    ...
    assert "--include-border-title" not in argv
...
async def test_argv_theme_optional(tmp_path: Path) -> None:
    stub = _write_kicad_cli_stub(tmp_path)
    pcb = _touch_pcb(tmp_path)
    await _make_tool(stub).run(PcbExportPdfInput(pcb_path=pcb, theme="KiCAD 2020"))
    argv = _read_argv(stub)
...
async def test_dry_run(tmp_path: Path) -> None:
    stub = _write_kicad_cli_stub(tmp_path)
    pcb = _touch_pcb(tmp_path)
    out = await _make_tool(stub).run(PcbExportPdfInput(pcb_path=pcb, dry_run=True))
...
async def test_pcb_not_found(tmp_path: Path) -> None:
    ...
    out = await _make_tool(stub).run(
        PcbExportPdfInput(pcb_path=tmp_path / "nope.kicad_pcb")
    )
...
async def test_wrong_suffix(tmp_path: Path) -> None:
    ...
    out = await _make_tool(stub).run(PcbExportPdfInput(pcb_path=f))
    assert out.status == "pcb_not_found"
...
async def test_cli_failed_nonzero_exit(tmp_path: Path) -> None:
    ...
    pcb = _touch_pcb(tmp_path)
    _stage_exit(stub, 11)
    out = await _make_tool(stub).run(PcbExportPdfInput(pcb_path=pcb))
...
async def test_without_injection_dry_run(tmp_path: Path) -> None:
    pcb = _touch_pcb(tmp_path)
    tool = PcbExportPdfTool()
    out = await tool.run(PcbExportPdfInput(pcb_path=pcb, dry_run=True))
    assert out.status == "dry_run"
...
def test_metadata() -> None:
    ...
    tool = PcbExportPdfTool()
    assert tool.name == "pcb_export_pdf"
